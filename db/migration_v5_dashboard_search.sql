-- ============================================================
-- KARATLY DASHBOARD - PART 7: Search + Unfulfilled Cashfree
-- Run against sabbpekaratly DB
--
-- Procedures:
--   sp_dashboard_search_clients(p_term)           -> client profile search
--   sp_dashboard_search_cashfree(p_term)          -> cashfree payments + gold_received
--   sp_dashboard_unfulfilled_cashfree(p_days, p_status) -> paid but gold not received
-- ============================================================

DELIMITER //

-- ============================================================
-- PART 7a: Search clients by name, phone, email, or uniqueId
-- ============================================================
DROP PROCEDURE IF EXISTS sp_dashboard_search_clients//
CREATE PROCEDURE sp_dashboard_search_clients(IN p_term VARCHAR(100))
BEGIN
    SELECT
        cp.client_id,
        cp.full_name,
        cp.email,
        cp.mobile,
        cp.provider_client_reference                       AS unique_id,
        cp.pan_number,
        cp.kyc_status,
        cp.bank_verified,
        DATE_FORMAT(cp.created_at, '%Y-%m-%d %H:%i:%s')   AS registered_at
    FROM client_profile cp
    WHERE cp.full_name LIKE CONCAT('%', p_term, '%')
       OR cp.mobile LIKE CONCAT('%', p_term, '%')
       OR cp.email LIKE CONCAT('%', p_term, '%')
       OR cp.provider_client_reference LIKE CONCAT('%', p_term, '%')
    ORDER BY cp.full_name
    LIMIT 50;
END//

-- ============================================================
-- PART 7b: Cashfree payments for matching users + gold_received flag
-- ============================================================
DROP PROCEDURE IF EXISTS sp_dashboard_search_cashfree//
CREATE PROCEDURE sp_dashboard_search_cashfree(IN p_term VARCHAR(100))
BEGIN
    SELECT
        cf.sabbpe_order_id,
        cf.merchant_order_id,
        cf.customer_id,
        cf.customer_name,
        cf.customer_mobile,
        cf.order_amount,
        cf.order_status,
        cf.remarks                                           AS fulfillment_status,
        p.payment_status,
        p.cf_payment_id,
        CASE WHEN cf.remarks LIKE 'FULFILLMENT_COMPLETED%'
             THEN 1 ELSE 0 END                              AS gold_received,
        DATE_FORMAT(cf.created_at, '%Y-%m-%d %H:%i:%s')    AS created_at
    FROM cashfreepg_orders cf
    LEFT JOIN cashfreepg_payments p ON p.sabbpe_order_id = cf.sabbpe_order_id
    WHERE cf.customer_id IN (
        SELECT cp.client_id FROM client_profile cp
        WHERE cp.full_name LIKE CONCAT('%', p_term, '%')
           OR cp.mobile LIKE CONCAT('%', p_term, '%')
           OR cp.email LIKE CONCAT('%', p_term, '%')
           OR cp.provider_client_reference LIKE CONCAT('%', p_term, '%')
    )
    ORDER BY cf.created_at DESC;
END//

-- ============================================================
-- PART 7c: Unfulfilled Cashfree orders (paid but gold not received)
-- ============================================================
DROP PROCEDURE IF EXISTS sp_dashboard_unfulfilled_cashfree//
CREATE PROCEDURE sp_dashboard_unfulfilled_cashfree(
    IN p_days INT,
    IN p_status VARCHAR(20)
)
BEGIN
    SELECT
        cf.sabbpe_order_id,
        cf.merchant_order_id,
        cf.customer_id,
        cf.customer_name,
        cf.customer_mobile,
        cf.order_amount,
        cf.order_status,
        cf.remarks                                           AS fulfillment_status,
        p.payment_status,
        p.cf_payment_id,
        cp.provider_client_reference                         AS provider_reference,
        DATE_FORMAT(cf.created_at, '%Y-%m-%d %H:%i:%s')    AS created_at
    FROM cashfreepg_orders cf
    LEFT JOIN cashfreepg_payments p  ON p.sabbpe_order_id = cf.sabbpe_order_id
    LEFT JOIN client_profile cp      ON cp.client_id      = cf.customer_id
    WHERE UPPER(p.payment_status) = 'SUCCESS'
      AND COALESCE(cf.remarks, '') NOT LIKE 'FULFILLMENT_COMPLETED'
      AND cf.created_at >= DATE_SUB(NOW(), INTERVAL p_days DAY)
      AND (
          p_status = 'ALL'
          OR (p_status = 'FAILED' AND UPPER(COALESCE(cf.remarks, '')) LIKE 'FULFILLMENT_FAILED%')
          OR (p_status = 'PENDING' AND (cf.remarks IS NULL OR TRIM(cf.remarks) = ''))
      )
    ORDER BY cf.created_at DESC;
END//

DELIMITER ;
