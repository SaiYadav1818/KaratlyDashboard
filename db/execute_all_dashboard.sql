...
--   PART 6 : MIS overview (sp_dashboard_mis_overview/aum/coupons — with month filter)
--
-- NOTE:
...
-- ============================================================
-- PART 7 : Search + Unfulfilled Cashfree
-- ============================================================

-- ============================================================
-- sp_dashboard_search_clients(p_term)
-- Search clients by name, phone, email, or uniqueId
-- ============================================================
DROP PROCEDURE IF EXISTS sp_dashboard_search_clients//
CREATE PROCEDURE sp_dashboard_search_clients(IN p_term VARCHAR(100))
BEGIN
    SELECT
        cp.client_id,
        cp.full_name,
        cp.email,
        cp.mobile,
        cp.provider_client_reference                       AS unique_id,
        cp.pan_number,
        cp.kyc_status,
        cp.bank_verified,
        DATE_FORMAT(cp.created_at, '%Y-%m-%d %H:%i:%s')   AS registered_at
...
-- ============================================================
-- sp_dashboard_search_cashfree(p_term)
-- Cashfree payments for matching users + gold_received flag
-- ============================================================
...
-- ============================================================
-- sp_dashboard_unfulfilled_cashfree(p_days, p_status)
-- Users who paid on Cashfree (SUCCESS) but gold not received
-- ============================================================
...
--
-- Procs:
--   sp_dashboard_mis_overview(p_month, p_year)  -> GMV + Transaction Count + Users + Metals + Coupons
...
proc: BEGIN
    ...
    LEFT JOIN (SELECT COUNT(*) AS ftd_count, SUM(COALESCE(o.total_amount, 0)) AS ftd_value FROM orders o WHERE o.order_type IN ('digital_purchase','digital_sell','physical_redemption','diamond_purchase') AND o.created_at >= v_mtd_start AND o.created_at <= v_ftd_end) ftd ON 1=1
    LEFT JOIN (
